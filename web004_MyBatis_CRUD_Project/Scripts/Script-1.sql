...
CREATE TABLE TEAM AS (
	SELECT TEAM_ID , REGION_NAME , TEAM_NAME , ORIG_YYYY , HOMEPAGE, STADIUM_ID
	FROM SQLD.TEAM
);

CREATE TABLE STADIUM AS (
	(SELECT STADIUM_ID , s.STADIUM_NAME AS STADIUM_NAME , HOMETEAM_ID AD TEAM_ID , SEAT_COUNT , t.ZIP_CODE1 AS ZIP_CODE1 , t.ZIP_CODE2 AS ZIP_CODE2 , t.ADDRESS AS ADDRESS , t.DDD AS DDD , t.TEL AS TEL , t.FAX AS FAX 
...
SELECT TO_CHAR(SYSDATE, 'YYYY')||LPAD(NVL(MAX(TO_NUMBER(SUBSTR(PLAYER_ID, 5, 3))), 0) + 1, 3, '0')
	FROM PLAYER
...
--INSERT INTO STADIUM(STADIUM_ID, STADIUM_NAME, SEAT_COUNT, ADDRESS)
--	VALUES('E02', '잠실', 15000, '서울특별시');

-- AD003(경기장 TEAM 수정) stadium_team_up
--UPDATE STADIUM SET HOMETEAM_ID='K88'
--	WHERE STADIUM_ID='E01';
