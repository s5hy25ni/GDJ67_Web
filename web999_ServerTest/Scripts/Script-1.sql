...
SELECT rt.SEQ, rt.ID, rt.TITLE, rt.CONTENT, rt.REGDATE, ORIGIN_FNAME , STORED_FNAME , FILE_SIZE, rp.TITLE RP_TITLE, rp.REGDATE RP_REGDATE 
			...
			ON rt.SEQ = B_SEQ LEFT JOIN (SELECT SEQ, "REF", TITLE, REGDATE
											FROM(SELECT SEQ, "REF", TITLE, REGDATE,
												ROW_NUMBER() OVER(ORDER BY SEQ) RN
												FROM REPLYBOARD)
											WHERE RN BETWEEN 1 AND 3) rp
			ON rt.SEQ = "REF"
			WHERE rt.SEQ = 5;
-- 댓글 총 개수
SELECT COUNT(*) TOTAL_REPLY
	FROM REPLYBOARD
	WHERE "REF"='1';
...
-- 파일 다운로드
SELECT ORIGIN_FNAME, STORED_FNAME
	FROM FILEBOARD
	WHERE B_SEQ=5;