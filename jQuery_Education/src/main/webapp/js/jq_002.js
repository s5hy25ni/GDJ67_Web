...
//a1() tag로 선택하기
function a1(){
	//jQuery에서는 tag로 탐색을 하면 NodeList로 처리해 준다
	//index번호를 eq(index), 만약 검색된 NodeList의 객체에 이벤트를 적용하면 모든 Node에 적용됨
	console.log($("span").length);
	$("span").css("color","green");
	// innerHTML, textContent = html(), text()
	$("#view").text("<b>글작성</b>");
...
function a3(){
	...
	$("#view").text('$(".t2").css("backgroundColor","yellow");');
}
...
// div[name=*'btn'] : div 중에서 name의 속성을 가지고 값이 btn을 포함하는 div를 선택
...
function a5(){
	$("li>b>span").css("backgroundColor","lime");
}
